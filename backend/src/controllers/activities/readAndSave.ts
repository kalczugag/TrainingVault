...
export const fetchAndSaveActivities = async (
    ...
    try {
        ...
        if (user.thresholdHistory && user.thresholdHistory.length > 0) {
            userFtp =
                user.thresholdHistory[user.thresholdHistory.length - 1].ftp ||
                250;
        }
        ...
        while (keepFetching) {
            ...
            for (const garminAct of activities) {
                ...
                if (!exists) {
                    ...
                    const power = safeNum(
                        rawAct.averagePower || rawAct.avgPower,
                    );
                    ...
                    if (finalTss === 0 && np > 0) {
                        ...
                        const rawTss =
                            ((duration * np * finalIf) / (userFtp * 3600)) *
                            100;
                    ...
                    await ActivityModel.create({
                        ...
                        summary: {
                            ...
                            anaerobicTe: safeFloat(
                                rawAct.anaerobicTrainingEffect,
                            ),
