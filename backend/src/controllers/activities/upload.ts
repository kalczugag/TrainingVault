...
export const uploadFitActivity = async (
    ...
    try {
        ...
        if (!file) {
            return res
                .status(400)
                .json(errorResponse(null, "No file uploaded", 400));
        }

        if (!file.originalname.toLowerCase().endsWith(".fit")) {
            return res
                .status(400)
                .json(errorResponse(null, "Only .fit files are allowed", 400));
        }
        ...
        if (
            ...
            return res
                .status(400)
                .json(errorResponse(null, "Invalid or empty FIT file", 400));
        }
            ...
            windows.forEach((w) => {
                if (powers.length >= w) {
                    ...
                    for (let i = w; i < powers.length; i++) {
                        currentSum = currentSum - powers[i - w] + powers[i];
        ...
        if (finalTss === 0 && np > 0) {
            const user = await (req.user as any).populate("thresholdHistory");
            let userFtp = 251;
            if (
                req.user &&
                (req.user as any).thresholdHistory &&
                (req.user as any).thresholdHistory.length > 0
            ) {
                const history = (req.user as any).thresholdHistory;
                userFtp = history[history.length - 1].ftp || 250;
            }
            finalIf = safeFloat(np / userFtp);
            const rawTss = ((duration * np * finalIf) / (userFtp * 3600)) * 100;
        ...
        const newActivity = await ActivityModel.create({
            ...
            title: `Wgrana aktywność (${activityDate.toLocaleDateString()})`,
            ...
            summary: {
                ...
                anaerobicTe: safeFloat(session.total_anaerobic_training_effect),
                ...
                max20MinPower:
                    safeNum(session.max_20_min_power) || powerCurve.p1200s,
        ...
        return res
            ...
            .json(
                successResponse(
                    newActivity,
                    "Activity, stream and stats uploaded successfully",
                ),
            );
    } catch (err: any) {
